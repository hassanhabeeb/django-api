...
from breathline.helpers.helper import get_object_or_none, get_token_user_or_none
...
class AMPMTimeField(serializers.TimeField):
    def to_representation(self, value):
        if value is None:
            return None
    ...
    def to_internal_value(self, data):
        if isinstance(data, str):
            ...
            raise serializers.ValidationError("Expected a string for time format.")
        
def convert_time_to_timezone(utc_time_str, timezone_str):
    ...
    # Combine current date and time into a single datetime object
    datetime_str = f'{current_date} {utc_time_str}'
    datetime_format = '%Y-%m-%d %H:%M:%S'
    
    # Parse the datetime string to a datetime object
    utc_time = datetime.strptime(datetime_str, datetime_format)

    # Set the timezone to UTC
    utc_time = pytz.utc.localize(utc_time)

    # Get the timezone object for the given timezone string
    target_timezone = pytz.timezone(timezone_str)

    # Convert the UTC time to the target timezone
    local_time = utc_time.astimezone(target_timezone)

    # Format the converted time to string (only time part)
    return {"date": local_time , "time" : local_time.strftime(time_format)}


def convert_time_to_timezone_lesson(date_str, time_str, timezone_str):
    time_format = '%H:%M:%S'
    date_format = '%Y-%m-%d'
    ...
    # Format the converted time to strings
    local_date_str = local_time.strftime(date_format)
    local_time_str = local_time.strftime(time_format)

    return {"date": local_date_str, "time": local_time_str}